import React from 'react';

const MusicCommunity = () => {
  return React.createElement('section', { className: "py-20 bg-slate-900 text-white" },
    React.createElement('div', { className: "container mx-auto px-6" },
      React.createElement('div', { className: "grid md:grid-cols-2 gap-16" },
        
        // Music Section
        React.createElement('div', null,
          React.createElement('div', { className: "flex items-center space-x-3 space-x-reverse mb-6" },
            React.createElement('div', { className: "p-3 bg-purple-600 rounded-lg" },
              React.createElement('svg', { xmlns: "http://www.w3.org/2000/svg", className: "h-6 w-6", fill: "none", viewBox: "0 0 24 24", stroke: "currentColor" },
                React.createElement('path', { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M9 19V6l12-3v13M9 19c0 1.105-1.343 2-3 2s-3-.895-3-2 1.343-2 3-2 3 .895 3 2zm12-3c0 1.105-1.343 2-3 2s-3-.895-3-2 1.343-2 3-2 3 .895 3 2zM9 10l12-3" })
              )
            ),
            React.createElement('h3', { className: "text-3xl font-bold" }, "EBEAT")
          ),
          React.createElement('p', { className: "text-slate-300 mb-6 text-lg" },
            "הצד היצירתי שבי. הפקות מוזיקליות שמטרתן היא לא רק לבדר, אלא לעורר מחשבה, מודעות ושינוי. המוזיקה היא שפה בינלאומית שמחברת בין אנשים לרעיונות גדולים."
          ),
          React.createElement('div', { className: "bg-slate-800 p-4 rounded-xl border border-slate-700" },
            React.createElement('div', { className: "flex items-center space-x-4 space-x-reverse" },
              React.createElement('div', { className: "w-12 h-12 bg-slate-700 rounded-full flex items-center justify-center" },
                React.createElement('svg', { xmlns: "http://www.w3.org/2000/svg", className: "h-6 w-6", viewBox: "0 0 20 20", fill: "currentColor" },
                  React.createElement('path', { fillRule: "evenodd", d: "M10 18a8 8 0 100-16 8 8 0 000 16zM9.555 7.168A1 1 0 008 8v4a1 1 0 001.555.832l3-2a1 1 0 000-1.664l-3-2z", clipRule: "evenodd" })
                )
              ),
              React.createElement('div', null,
                React.createElement('h4', { className: "font-bold" }, "Latest Release"),
                React.createElement('p', { className: "text-sm text-slate-400" }, "Earth Rhythm - 2024 Mix")
              )
            )
          )
        ),

        // Community Section
        React.createElement('div', null,
          React.createElement('div', { className: "flex items-center space-x-3 space-x-reverse mb-6" },
            React.createElement('div', { className: "p-3 bg-yellow-500 rounded-lg text-white" },
              React.createElement('svg', { xmlns: "http://www.w3.org/2000/svg", className: "h-6 w-6", fill: "none", viewBox: "0 0 24 24", stroke: "currentColor" },
                React.createElement('path', { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" })
              )
            ),
            React.createElement('h3', { className: "text-3xl font-bold" }, "קהילה והתנדבות")
          ),
          React.createElement('p', { className: "text-slate-300 mb-6 text-lg" },
            "אי אפשר לעשות שינוי לבד. פעילות מתמדת לשיקום הטבע המקומי ויצירת חיבורים בקהילה. כי האקולוגיה מתחילה בבית, באנשים שסביבנו."
          ),
          React.createElement('div', { className: "grid grid-cols-2 gap-4" },
            React.createElement('div', { className: "bg-slate-800 p-4 rounded-xl border border-slate-700 text-center" },
              React.createElement('span', { className: "block text-2xl font-bold text-yellow-500" }, "20+"),
              React.createElement('span', { className: "text-sm text-slate-400" }, "פרויקטים קהילתיים")
            ),
            React.createElement('div', { className: "bg-slate-800 p-4 rounded-xl border border-slate-700 text-center" },
              React.createElement('span', { className: "block text-2xl font-bold text-yellow-500" }, "100%"),
              React.createElement('span', { className: "text-sm text-slate-400" }, "התנדבות")
            )
          )
        )

      )
    )
  );
};

export default MusicCommunity;
